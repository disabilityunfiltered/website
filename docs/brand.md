...
Sponsor logo (homepage sponsor strip): src/assets/sponsor-hearts-in-action-{300,375,450,525,600,900}.png, served via srcset at 1x/1.25x/1.5x/1.75x/2x/3x (exact sizes for every common Windows scaling; a missing density forces browser resampling, which blurs), shown 300px wide max, linking to heartsinaction.com.au. Master: hearts-in-action-horizontal-small.svg (stroke-thickened small lockup) in Documents\Hearts In Action\Logo & Social Media\Logo - Redone. Regenerate: render the SVG at 2400px (headless Chrome, transparent), Lanczos-downscale to each width, unsharp-mask all but the 900 (percent 110/90/70/60/50 for 300/375/450/525/600, radius 1).
