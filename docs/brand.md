...
Sponsor logo (homepage sponsor strip): src/assets/sponsor-hearts-in-action-{570,713,855,998,1140,1710}.png, served via srcset at 1x/1.25x/1.5x/1.75x/2x/3x (exact sizes for every common Windows scaling; a missing density forces browser resampling, which blurs). Shown 570px wide max, linking to heartsinaction.com.au. Lockup rule (Matt, 2026-08-24): wordmark height = 2/3 of heart icon height, at large wordmark size. Icon and wordmark are rendered SEPARATELY from the master SVG at their exact final scales (never resize rasters up, it patches the gradients), then composed: gap 0.135 x icon height, text centre at 54% of icon height. Master: hearts-in-action-horizontal-from-ai.svg in Documents\Hearts In Action\Logo & Social Media\Logo - Redone: the designer's true vectors, converted from "logo ai.ai" with Inkscape (--export-plain-svg --pdf-poppler), viewBox cropped to the horizontal lockup, the 14 solid-colour letter paths stroke-thickened 2.7 units (letters are flat colours in the original, so stroking cannot seam; the gradient heart is NEVER stroked). Regenerate: render the SVG at ~2448px (headless Chrome, transparent), Lanczos-downscale to each width. NO unsharp mask: sharpening mottles gradient fills.
