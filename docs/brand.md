...
Sponsor logo (homepage sponsor strip): src/assets/sponsor-hearts-in-action-{360,450,540,630,720,1080}.png, served via srcset at 1x/1.25x/1.5x/1.75x/2x/3x (exact sizes for every common Windows scaling; a missing density forces browser resampling, which blurs). Shown 360px wide max, linking to heartsinaction.com.au. Master: the designer original "logo PNG 2.png" (9912x2294) in Documents\Hearts In Action\Logo & Social Media\Logo. Do NOT use the "Logo - Redone" SVGs: their hearts are segmented gradients that render blotchy. Regenerate: Lanczos-downscale the master to each width, unsharp-mask all but the 1080 (percent 70/60/50/45/40 for 360/450/540/630/720, radius 1).
