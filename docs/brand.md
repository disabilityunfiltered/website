...
Sponsor logo (homepage sponsor strip): src/assets/sponsor-hearts-in-action-{360,450,540,630,720,1080}.png, served via srcset at 1x/1.25x/1.5x/1.75x/2x/3x (exact sizes for every common Windows scaling; a missing density forces browser resampling, which blurs). Shown 360px wide max, linking to heartsinaction.com.au. Master: hearts-in-action-horizontal-small.svg in Documents\Hearts In Action\Logo - Redone (wordmark stroke-thickened for small sizes; the heart is NEVER stroked, its segmented gradients smear into blotches). Regenerate: render the SVG at 2400px (headless Chrome, transparent), Lanczos-downscale to each width. NO unsharp mask: sharpening mottles the gradient fills.
