...
- Episodes are Markdown files in `content/episodes/`. `content/episodes/episodes.11tydata.js` gives every file the `layouts/episode.njk` layout and a `/episodes/[file-name]/` URL. The layout reads all media (audioUrl, youtubeId, listen.*) from the file's front matter, so the page is podcast-host-agnostic; body = transcript. The `episodes` collection is defined in `eleventy.config.js` (glob on that folder, newest first). Field reference: docs/content-workflow.md. Adding an episode never touches code.
...
- One CSS file per component, same base name as its template (header.css/header.njk). Page-scoped styles use the page's name (home.css styles index.njk's hero). `base.css` holds reset, typography, buttons, focus states, utilities (`.visually-hidden` for screen-reader-only headings).
...
`eleventy.config.js`: passthrough copy for css/js/assets/robots.txt, episodes collection, filters `readableDate` (en-AU), `isoDate` (YYYY-MM-DD for datetime attributes), `toSeconds` ("h:mm:ss" to seconds for chapter links), `year` shortcode. `.eleventyignore` keeps docs/, workers/, PLAN.md, CLAUDE.md, README.md out of the build.
